...
DLLEXPORT void __stdcall PublicKeyToString(EC_POINT* public_key, EC_GROUP* group,
	ECDSAPublicKey* key) {
	BIGNUM* x = BN_new();  // x座標
	BIGNUM* y = BN_new();  // y座標
	EC_POINT_get_affine_coordinates(group, public_key, x, y, NULL);
...
DLLEXPORT void __stdcall StringToPublicKey(EC_POINT* public_key, EC_GROUP* group,
	ECDSAPublicKey* key) {
	BIGNUM* x = BN_new();  // x座標
	BIGNUM* y = BN_new();  // y座標
...
DLLEXPORT void __stdcall BIGNUMToString(BIGNUM* bn, char* str) {
	strcpy_s(str, HEX_UNSIGNED_SIZE, BN_bn2hex(bn));
	return;
}

DLLEXPORT void __stdcall StringToBIGNUM(BIGNUM* bn, char* str) {
	BN_hex2bn(&bn, str);
	return;
}

DLLEXPORT void __stdcall GetPublicKeyStrings(ECDSAPublicKey* publicKeyStrings[], int size) {
...
DLLEXPORT BIGNUM* __stdcall H_function(char message[], EC_POINT* r_point, EC_GROUP* group) {
	// メッセージのハッシュ値を計算
...
DLLEXPORT BIGNUM* __stdcall sk_function(EC_GROUP* group, BIGNUM* alpha, BIGNUM* ck, BIGNUM* nk) {
	BIGNUM* sk = BN_new();
...
// ランダムなBIGNUMを生成する関数
DLLEXPORT BIGNUM* __stdcall get_randNum(EC_GROUP* group) {
	unsigned char rand_hash[SHA256_DIGEST_LENGTH];
	RAND_bytes(rand_hash, SHA256_DIGEST_LENGTH);
